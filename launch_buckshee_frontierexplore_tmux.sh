...
case "$1" in
    ...
    --tab8)
        ...
        do_ssh "mkdir -p /mnt/nova_ssd/workspaces/isaac_ros-dev/experiment_logs; tmux has-session -t bk_mesh_zenoh 2>/dev/null || tmux new-session -d -x 220 -y 50 -s bk_mesh_zenoh \"sudo bash /mnt/nova_ssd/workspaces/isaac_ros-dev/launch_mesh_zenoh.sh $NODE_ID $PEER_IDS\"; tmux pipe-pane -o -t bk_mesh_zenoh \"cat >> /mnt/nova_ssd/workspaces/isaac_ros-dev/experiment_logs/pane_bk_mesh_zenoh_\$(date +%s).log\"; tmux attach-session -t bk_mesh_zenoh"
        echo ""
