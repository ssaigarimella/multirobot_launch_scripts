...
case "$1" in
    ...
    --tab8)
        ...
        do_ssh "mkdir -p /mnt/nova_ssd/workspaces/isaac_ros-dev/experiment_logs; tmux has-session -t ts_mesh_zenoh 2>/dev/null || tmux new-session -d -x 220 -y 50 -s ts_mesh_zenoh \"sudo bash /mnt/nova_ssd/workspaces/isaac_ros-dev/launch_mesh_zenoh.sh $NODE_ID $PEER_IDS\"; tmux pipe-pane -o -t ts_mesh_zenoh \"cat >> /mnt/nova_ssd/workspaces/isaac_ros-dev/experiment_logs/pane_ts_mesh_zenoh_\$(date +%s).log\"; tmux attach-session -t ts_mesh_zenoh"
        echo ""
